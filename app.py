...
def user_ready(user):
    ...
    for archetype, possible_choices in task_archetypes.items():
        current_in_use = Tasknames.query.with_entities(Tasknames.task_name).filter_by(game_id="game1", task_archetype=archetype).all()
        available = set(possible_choices) - set(current_in_use)
        mapping = Tasknames(game_id="game1", player_id=user.player_id, task_archetype=archetype, task_name=random.sample(available, 1)[0])
...
def check_start(game_id, user):
    ...
    if len(ready_players) == NUM_PLAYERS:
        ...
        mappings = Tasknames.query.with_entities(Tasknames.task_archetype, Tasknames.task_name).filter_by(game_id="game1", player_id=user).all()

        #get tasks that aren't assigned to the user (to be yelled)
        potential_tasks = Tasknames.query.with_entities(Tasknames.id, Tasknames.task_archetype, Tasknames.task_name).filter(Tasknames.player_id != user).all() #TODO: missing game1 filter, add here
        ...
        for id, archetype, name in potential_tasks:
            if id not in used_ids:
                ...
                assignment = TaskAssignments(id=id, game_id="game1", player_id=user, task_archetype=archetype, task_name=name)
