...
void loop()
{
  ...
  {
    primary_timer = millis();
    char *json_response = get_new_round(game_id, player_name, request, response, OUT_BUFFER_SIZE, RESPONSE_TIMEOUT);
  ...
  //  Since we are only doing one round at the moment, if the game_status is false then the game ended! :)
  else
  {
